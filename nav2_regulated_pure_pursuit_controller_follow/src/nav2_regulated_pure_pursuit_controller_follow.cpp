...
#include "nav2_regulated_pure_pursuit_controller_follow/nav2_regulated_pure_pursuit_controller_follow.hpp"
...
namespace nav2_regulated_pure_pursuit_controller_follow
{
  ...
  const geometry_msgs::msg::Twist & speed)
{
  ...
  auto carrot_pose = getLookAheadPoint(lookahead_dist, global_plan_);
...
void RegulatedPurePursuitController::transformGlobalPlan()
{
  ...
  for( auto & pose : global_plan_.poses) {
    ...
    if (!transformPose(costmap_ros_->getBaseFrameID(), pose, pose)) {
      throw nav2_core::PlannerException("Could not transform plan");
    }
  }
}
...
PLUGINLIB_EXPORT_CLASS(
  nav2_regulated_pure_pursuit_controller_follow::RegulatedPurePursuitController,
